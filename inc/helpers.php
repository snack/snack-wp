 ...
 * Add in single.php <?php snack_related_posts(); ?>
 * To show related by tags:
 * Add in single.php <?php snack_related_posts( 'tag' ); ?>
 *
 * @since  2.2.0
 *
 ...
 */
function snack_related_posts( $display = 'category', $qty = 4, $title = '', $thumb = true, $post_type = 'post' ) {
	global $post;
		...
		if ( $related->have_posts() ) {
			...
			while ( $related->have_posts() ) {
				...
				if ( $thumb ) {
					...
					$image = apply_filters( 'snack_related_posts_thumbnail', $img );
 ...
 *
 * Usage:
 * Place: <?php echo snack_excerpt( 'excerpt', value ); ?>
 *
 * @since  2.2.0
 *
 ...
 */
function snack_excerpt( $type = 'excerpt', $limit = 40 ) {
	$limit = (int) $limit;
 ...
 */
function snack_breadcrumbs( $homepage = '' ) {
 ...
 * @return string
 */
function snack_get_image_url( $id, $width, $height, $crop = true, $upscale = false ) {
	$resizer    = snack_Thumbnail_Resizer::get_instance();
 ...
 *
 * Use:
 * add_action( 'the_post', 'snack_autoset_featured' );
 * add_action( 'save_post', 'snack_autoset_featured' );
 * add_action( 'draft_to_publish', 'snack_autoset_featured' );
 * add_action( 'new_to_publish', 'snack_autoset_featured' );
 * add_action( 'pending_to_publish', 'snack_autoset_featured' );
 * add_action( 'future_to_publish', 'snack_autoset_featured' );
 *
 * @since  2.2.0
 *
 * @global array $post WP post object.
 */
function snack_autoset_featured() {
	global $post;
