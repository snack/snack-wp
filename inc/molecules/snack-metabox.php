...
class Snack_Metabox {
    ...
    public function metabox( $post ) {
        ...
        do_action( 'snack_metabox_header_' . $this->id, $post_id );

        echo apply_filters( 'snack_metabox_container_before_' . $this->id, '<table class="form-table odin-form-table">' );

        foreach ( $this->fields as $field ) {
            echo apply_filters( 'snack_metabox_wrap_before_' . $this->id, '<tr valign="top">', $field );
            ...
            echo apply_filters( 'snack_metabox_field_title_' . $this->id, $title, $field );

            echo apply_filters( 'snack_metabox_field_before_' . $field['id'], '<td>', $field );
            ...
            echo apply_filters( 'snack_metabox_field_after_' . $field['id'], '</td>', $field );

            echo apply_filters( 'snack_metabox_wrap_after_' . $this->id, '</tr>', $field );
        }

        echo apply_filters( 'snack_metabox_container_after_' . $this->id, '</table>' );

        do_action( 'snack_metabox_footer_' . $this->id, $post_id );
    ...
    protected function process_fields( $args, $post_id ) {
        ...
        switch ( $type ) {
            ...
            default:
                do_action( 'snack_metabox_field_' . $this->id, $type, $id, $current, $options, $attrs );
                break;
        }
    }
    ...
    protected function field_image( $id, $current ) {
        ...
        $image = apply_filters( 'snack_placeholder_path', get_template_directory_uri() . '/core/assets/images/placeholder.png' );
    ...
    public function save( $post_id ) {
        ...
        foreach ( $this->fields as $field ) {
            ...
            if ( ! in_array( $field['type'], array( 'separator', 'title' ) ) ) {
                ...
                $new = apply_filters( 'snack_save_metabox_' . $this->id, $value, $name );
